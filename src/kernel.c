...
load_file(mrb_state *mrb, mrb_value filename)
{
    ...
    //fp = fopen(mrbc_filename(mrb, c, RSTRING_PTR(str)), "r");
    //fp = fopen(c->filename, "r");
    fp = fopen(RSTRING_PTR(filename), "r");
    if (!fp) {
        fprintf(stderr,"file %s not found\n",RSTRING_PTR(filename));
        mrbc_context_free(mrb, c);
        mrb_raise(mrb, E_LOAD_ERROR, "file '%s' not found.", RSTRING_PTR(str));
    }
...
;     /* 15.3.1.2.11 */
  ...
  mrb_define_method(mrb, krn, "require",                    mrb_f_require,                   ARGS_REQ(1));    /* 15.3.1.3.47 */
  mrb_define_method(mrb, krn, "load",                       mrb_f_load,                      ARGS_REQ(1));    /* 15.3.1.3.48 */
